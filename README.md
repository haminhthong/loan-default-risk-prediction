...
- Đánh giá bằng PR-AUC, ROC-AUC, F1, recall, precision và balanced accuracy.
...
Lần kiểm thử với `random_state=42`, Logistic Regression và cách chia 70/15/15 cho
...
| PR-AUC | 0,2793 |
| ROC-AUC | 0,7059 |
| Recall nợ xấu | 0,6850 |
| Precision nợ xấu | 0,2387 |
| F1 | 0,3540 |
| Balanced accuracy | 0,6612 |
| Ngưỡng chọn trên validation | 0,50 |
...
```bash
python -m src.train --data "data/Bank Loan Dataset.csv" --output models/loan_default.joblib
```
...
5. Fit mô hình trên train.
6. Chọn ngưỡng trên validation theo chi phí false negative/false positive.
7. Mở test đúng một lần để báo cáo cuối cùng.
8. Lưu pipeline, danh sách cột, định nghĩa nhãn, ngưỡng và metric trong một artifact.
